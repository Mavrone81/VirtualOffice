...
const TYPES: { v: DocumentType; label: string }[] = [
  { v: "CompanyTemplate", label: "Company Template" },
  { v: "AssociateAgreement", label: "Associate Agreement" },
  { v: "VendorAgreement", label: "Vendor Agreement" },
  { v: "VendorMOU", label: "Vendor MOU" },
  { v: "SalesAgreement", label: "Sales Agreement" },
  { v: "Other", label: "Other" },
];
...
export function DocumentForm() {
  ...
  return (
    <Card className="p-5">
      <h2 className="mb-4 font-display text-[17px] text-ink">Upload document</h2>
      <div className="space-y-4">
        ...
        <div className="grid gap-4 sm:grid-cols-2">
          ...
          <div>
            <Label htmlFor="as">Shared with</Label>
            <select id="as" className={selectCls} value={assignment} onChange={(e) => setAssignment(e.target.value as "All" | "Team" | "Associate")}>
              ...
              <option value="Associate">One associate</option>
            </select>
          </div>
          {assignment === "Team" && (
            <div>
          ...
          {assignment === "Associate" && (
            <div>
              <Label htmlFor="ac">Associate code</Label>
        ...
        <div>
          ...
          <p className="mt-1 text-[12px] text-muted-2">PDF, image, or Office file, up to 15 MB.</p>
        </div>
        {error && <p className="rounded-lg bg-danger-50 px-3 py-2 text-[13px] text-danger">{error}</p>}
        <Button onClick={submit} disabled={pending || !title || !file}>{pending ? "Uploading…" : "Upload document"}</Button>
      </div>
    </Card>
  );
}
