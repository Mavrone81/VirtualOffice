...
export default async function AuditLogPage() {
  ...
  return (
    <>
      <PageHeader title="Audit log" subtitle="Trail of security- and finance-sensitive actions." />

      <Card className="overflow-hidden">
        {logs.length === 0 ? (
          <p className="px-5 py-12 text-center text-[13px] text-muted">No audit entries yet.</p>
        ) : (
                ...
                {logs.map((l) => {
                  ...
                  return (
                    <tr key={l.id} className="border-b border-line-200 last:border-0 hover:bg-paper-100">
                      ...
                      <td className="px-5 py-3 text-muted">{l.actorUserId ? emailOf.get(l.actorUserId) ?? "—" : "system"}</td>
