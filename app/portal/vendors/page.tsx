...
export default async function PortalVendorsPage() {
  ...
  return (
    <>
      <PageHeader title={t("vendors.pageTitle")} subtitle={t("vendors.pageSubtitle")} />
      ...
      <div className="grid gap-6 lg:grid-cols-[1fr_1.3fr]">
        ...
        <Card className="overflow-hidden">
          <div className="border-b border-line px-5 py-4">
            <h2 className="font-display text-[17px] text-ink">{t("vendors.activeVendors", { count: vendors.length })}</h2>
          </div>
          {vendors.length === 0 ? (
            <p className="px-5 py-12 text-center text-[13px] text-muted">{t("vendors.noVendors")}</p>
          ) : (
